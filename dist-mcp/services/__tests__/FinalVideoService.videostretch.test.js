 ...
 */
(0, vitest_1.describe)('FinalVideoService - Video Stretching Fix (Frozen Frames)', () => {
    const calculateSetpts = (videoSpeed) => {
    ...
    (0, vitest_1.it)('should lengthen video when videoSpeed < 1.0', () => {
        ...
        (0, vitest_1.expect)(setpts).toContain('1.1539');
        (0, vitest_1.expect)(setpts).toContain('*PTS');
        // Verify multiplier is greater than 1.0 (longer output)
        const multiplier = parseFloat(setpts.split('*')[0]);
        (0, vitest_1.expect)(multiplier).toBeGreaterThan(1.0);
    });
    (0, vitest_1.it)('should shorten video when videoSpeed > 1.0', () => {
        const videoSpeed = 1.25;
        const setpts = calculateSetpts(videoSpeed);
        // Expected: setpts = (1/1.25)*PTS = 0.8000*PTS (shorter output)
        (0, vitest_1.expect)(setpts).toContain('0.8000');
        (0, vitest_1.expect)(setpts).toContain('*PTS');
        // Verify multiplier is less than 1.0 (shorter output)
        const multiplier = parseFloat(setpts.split('*')[0]);
        (0, vitest_1.expect)(multiplier).toBeLessThan(1.0);
    });
    (0, vitest_1.it)('should not change speed when videoSpeed = 1.0', () => {
        const videoSpeed = 1.0;
        const setpts = calculateSetpts(videoSpeed);
        // Expected: Just reset PTS, no speed change
        (0, vitest_1.expect)(setpts).toBe('PTS-STARTPTS');
    });
    (0, vitest_1.it)('should handle extreme slow motion (videoSpeed = 2.0)', () => {
        // Video needs to be stretched 2x
        const videoSpeed = 2.0;
        const setpts = calculateSetpts(videoSpeed);
        // Expected: setpts = 0.5*PTS (half speed = slow motion)
        (0, vitest_1.expect)(setpts).toContain('0.5000');
        const multiplier = parseFloat(setpts.split('*')[0]);
        (0, vitest_1.expect)(multiplier).toBe(0.5);
    });
    (0, vitest_1.it)('should handle extreme speed up (videoSpeed = 0.5)', () => {
        // Video needs to be compressed 2x
        const videoSpeed = 0.5;
        const setpts = calculateSetpts(videoSpeed);
        // Expected: setpts = 2.0*PTS (double speed)
        (0, vitest_1.expect)(setpts).toContain('2.0000');
        const multiplier = parseFloat(setpts.split('*')[0]);
        (0, vitest_1.expect)(multiplier).toBe(2.0);
    });
    (0, vitest_1.it)('should calculate correct duration after setpts', () => {
        ...
        (0, vitest_1.expect)(resultDuration1).toBeCloseTo(11.54, 1);
        ...
        (0, vitest_1.expect)(resultDuration2).toBeCloseTo(8.0, 1);
    });
    (0, vitest_1.it)('should handle videoSpeed very close to 1.0', () => {
    ...
    (0, vitest_1.it)('should handle videoSpeed = 0.9999 (very close to 1.0)', () => {
        const videoSpeed = 0.9999;
        const setpts = calculateSetpts(videoSpeed);
        // Should be treated as no change (within threshold)
        (0, vitest_1.expect)(setpts).toBe('PTS-STARTPTS');
    });
    (0, vitest_1.it)('should verify setpts formula correctness', () => {
        ...
        testCases.forEach(({ videoSpeed, expectedMultiplier }) => {
            const setpts = calculateSetpts(videoSpeed);
            const multiplier = parseFloat(setpts.split('*')[0]);
            (0, vitest_1.expect)(multiplier).toBeCloseTo(expectedMultiplier, 3);
        });
    });
    (0, vitest_1.it)('should build correct filter string', () => {
        const buildFilterString = (start, end, videoSpeed, fps) => {
        ...
        (0, vitest_1.expect)(filter).toContain('trim=start=0.0000:end=10.0000');
        (0, vitest_1.expect)(filter).toContain('setpts=0.8666*PTS');
        (0, vitest_1.expect)(filter).toContain('fps=30.000');
    });
    (0, vitest_1.it)('should not cause frozen frames with correct setpts', () => {
        ...
        (0, vitest_1.expect)(multiplier).toBeGreaterThan(1.0);
        (0, vitest_1.expect)(multiplier).toBeCloseTo(1.5, 3);
        // This will NOT cause frozen frames
    });
});
//# sourceMappingURL=FinalVideoService.videostretch.test.js.map