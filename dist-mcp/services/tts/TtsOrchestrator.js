...
exports.TtsOrchestrator = void 0;
const p_limit_1 = __importDefault(require("p-limit"));
const TtsErrorClassifier_1 = require("./TtsErrorClassifier");
function adjustConcurrency(stats) {
...
class TtsOrchestrator {
    engine;
    defaultConcurrency;
    constructor(engine, concurrency = 10) {
        this.engine = engine;
        this.defaultConcurrency = concurrency;
    }
    async generateSegment(text, voiceId, outputPath, maxRetries = 2) {
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                ...
                return { success: false, error: (0, TtsErrorClassifier_1.categorizeTtsError)(err) };
            }
            if (attempt < maxRetries) {
                const delay = Math.pow(2, attempt) * 1000;
                console.log(`Retry ${attempt + 1}/${maxRetries} for ${outputPath} after ${delay}ms`);
    ...
    async generateBatch(entries, voiceId, getOutputPath, concurrency, signal, onProgress) {
        if (signal?.aborted)
            return [];
        const results = new Array(entries.length).fill('');
        const stats = {
            successCount: 0,
            failCount: 0,
        ...
        const limit = (0, p_limit_1.default)(currentConcurrency);
        let completed = 0;
        ...
        for (let round = 0; round < 2; round++) {
            ...
            const retryLimit = (0, p_limit_1.default)(currentConcurrency);
            const retryTasks = failedIndices.map((i) => retryLimit(async () => {
...
exports.TtsOrchestrator = TtsOrchestrator;
//# sourceMappingURL=TtsOrchestrator.js.map