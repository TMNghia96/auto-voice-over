...
exports.SegmentValidator = void 0;
/**
...
class SegmentValidator {
     ...
     */
    validateAndAdjust(segments, actualAudioDurations, videoDuration) {
        // Validate input lengths match
        if (segments.length !== actualAudioDurations.length) {
            throw new Error(`Segments and actualAudioDurations length mismatch: ${segments.length} vs ${actualAudioDurations.length}`);
        }
        return segments.map((segment, index) => {
            ...
            const adjustedVideoSpeed = this.roundToDecimal(originalVideoDuration / actualAudioDuration, 4);
            // Check if segment is beyond video duration
            if (segment.videoStart >= videoDuration) {
                console.warn(`Segment ${index}: videoStart ${segment.videoStart} is beyond video duration ${videoDuration}. ` +
                    `Will need to create freeze frame or black video.`);
            }
            // Validate speed bounds
            if (adjustedVideoSpeed < this.MIN_SPEED) {
                console.warn(`Segment ${index}: adjusted speed ${adjustedVideoSpeed.toFixed(2)} is below minimum ${this.MIN_SPEED}. ` +
                    `Video may appear too slow.`);
            }
            if (adjustedVideoSpeed > this.MAX_SPEED) {
                console.warn(`Segment ${index}: adjusted speed ${adjustedVideoSpeed.toFixed(2)} is above maximum ${this.MAX_SPEED}. ` +
...
//# sourceMappingURL=SegmentValidator.js.map