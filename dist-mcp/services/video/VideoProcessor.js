...
class VideoProcessor {
    ...
    async processVideoChunks(chunks, originalVideo, tempDir, onProgress, forceEncode = false) {
        if (chunks.length === 0)
            return [];
        const shouldEncode = (chunk) => forceEncode || Math.abs(chunk.adjustedVideoSpeed - 1.0) > VideoProcessor.SPEED_EPSILON;
        ...
        const workLimit = (0, p_limit_1.default)(concurrency);
        let completed = 0;
            ...
            return workLimit(async () => {
                try {
                    ...
                    if (actualDuration > 0 && Math.abs(actualDuration - expectedDuration) > 0.3) {
                        if (!shouldEncode(chunk)) {
                            console.warn(`Copy chunk ${index} duration mismatch: expected ${expectedDuration.toFixed(3)}s, got ${actualDuration.toFixed(3)}s. Re-encoding chunk.`);
                            await this.encodeChunk(await getEncoder(), chunk, index, originalVideo, out);
                            actualDuration = await this.getMediaDuration(out);
                        }
                        if (Math.abs(actualDuration - expectedDuration) > 0.3) {
                            console.warn(`Chunk ${index} duration mismatch after processing: expected ${expectedDuration.toFixed(3)}s, got ${actualDuration.toFixed(3)}s (non-fatal)`);
                        }
                    }
                    completed++;
    ...
    async normalizeVideoDuration(inputPath, outputPath, expectedDuration, actualDuration) {
        const encoder = await this.encoderFactory.createEncoder();
        const useGPU = encoder.type === 'gpu';
