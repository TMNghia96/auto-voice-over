 ...
 */
describe('FinalVideoService - Fade Expression Fix (Bug #3)', () => {
    interface Segment {
    ...
    it('should handle normal duration segments', () => {
        const seg: Segment = { targetDuration: 5.0, fadeStart: true, fadeEnd: true };
        const expr = createFadeExpression(seg, 0.15, 0.5);
        
        expect(expr).toContain('if(lt(t,0.500)');
        expect(expr).toContain('if(lt(t,4.500)');
        expect(validateFadeExpression(expr)).toBe(true);
    });
    
    it('should reduce fade duration for short segments', () => {
        const seg: Segment = { targetDuration: 0.8, fadeStart: true, fadeEnd: true };
        const expr = createFadeExpression(seg, 0.15, 0.5);
        
        // Adjusted fade should be (0.8 - 0.1) / 2 = 0.35
        expect(expr).toContain('0.350');
        expect(validateFadeExpression(expr)).toBe(true);
    });
    
    it('should return no fade for very short segments', () => {
        const seg: Segment = { targetDuration: 0.15, fadeStart: true, fadeEnd: true };
        const expr = createFadeExpression(seg, 0.15, 0.5);
    ...
    it('should handle fade in only', () => {
        const seg: Segment = { targetDuration: 3.0, fadeStart: true, fadeEnd: false };
        const expr = createFadeExpression(seg, 0.15, 0.5);
        
        expect(expr).toContain('if(lt(t,0.500)');
        expect(expr).not.toContain('if(lt(t,2.500)');
        expect(validateFadeExpression(expr)).toBe(true);
    });
    
    it('should handle fade out only', () => {
        const seg: Segment = { targetDuration: 3.0, fadeStart: false, fadeEnd: true };
        const expr = createFadeExpression(seg, 0.15, 0.5);
        
        expect(expr).toContain('if(lt(t,2.500)');
        expect(expr).not.toContain('0.15+');
        expect(validateFadeExpression(expr)).toBe(true);
    });
    
    it('should validate expression length', () => {
        const longExpr = 'if('.repeat(100) + '1.0' + ')'.repeat(100);
        expect(validateFadeExpression(longExpr)).toBe(false);
    });
    
    it('should validate balanced parentheses', () => {
        expect(validateFadeExpression('if(lt(t,0.5),1.0,0.5)')).toBe(true);
        expect(validateFadeExpression('if(lt(t,0.5),1.0,0.5')).toBe(false);
        expect(validateFadeExpression('if(lt(t,0.5)),1.0,0.5)')).toBe(false);
    });
    
    it('should not overlap fades', () => {
        const seg: Segment = { targetDuration: 1.0, fadeStart: true, fadeEnd: true };
        const expr = createFadeExpression(seg, 0.15, 0.5);
    ...
    it('should handle no fade case', () => {
        const seg: Segment = { targetDuration: 3.0, fadeStart: false, fadeEnd: false };
        const expr = createFadeExpression(seg, 0.15, 0.5);
        
        expect(expr).toBe('1.0');
    });
    
    it('should handle different duck volumes', () => {
        const seg: Segment = { targetDuration: 3.0, fadeStart: true, fadeEnd: false };
        ...
        expect(expr1).toContain('0.10');
        expect(expr2).toContain('0.20');
        expect(expr3).toContain('0.50');
        
        expect(validateFadeExpression(expr1)).toBe(true);
        expect(validateFadeExpression(expr2)).toBe(true);
        expect(validateFadeExpression(expr3)).toBe(true);
    });
    
    it('should handle different fade durations', () => {
        const seg: Segment = { targetDuration: 5.0, fadeStart: true, fadeEnd: false };
        ...
        expect(expr1).toContain('0.300');
        expect(expr2).toContain('0.500');
        expect(expr3).toContain('1.000');
    });
    
    it('should handle edge case: exactly minimum duration', () => {
        const fadeDuration = 0.5;
        const minDuration = fadeDuration * 2 + 0.1; // 1.1s
        const seg: Segment = { targetDuration: minDuration, fadeStart: true, fadeEnd: true };
        
        const expr = createFadeExpression(seg, 0.15, fadeDuration);
        
        expect(validateFadeExpression(expr)).toBe(true);
        expect(expr).toContain('0.500'); // Should use full fade duration
    });
    ...
    it('should validate complex nested expressions', () => {
        ...
        expect(validateFadeExpression(validExpr)).toBe(true);
        
        const invalidExpr = 'if(lt(t,0.500),0.15+(0.85)*t/0.500,if(lt(t,4.500),1.0,1.0-(0.85)*(t-4.500)/0.500)';
        expect(validateFadeExpression(invalidExpr)).toBe(false);
    });
});
